...
#define ENABLE_SERVO 0     // servo
#define ENABLE_LCD 0       // lcd
#define ENABLE_LED_STRIP 0 // leds (Neopixels)
#define ENABLE_DHT 0       // dht

#define ENABLE_IR_REMOTE 0 // ir
#define ENABLE_RFID_UART 0 // rfid (SoftwareSerial)
// #define ENABLE_BT_UART 1   // bluetooth (SoftwareSerial)
#define ENABLE_STEPPER 0    // stepper
#define ENABLE_LED_MATRIX 0 // matrix (MD_MAX72XX)
#define ENABLE_TM 0         // Digital Display
...
namespace PinManager
{
  ...
  inline int lookup(const char *in)
  {
    ...
    if (in[0] == 'A' || in[0] == 'a')
    {
      ...
      if (idx < 0 || idx >= NUM_ANALOG_INPUTS)
        return -1;
      return A0 + (int)idx;
    }
    if (in[0] == 'D' || in[0] == 'd')
  ...
  inline bool reserve(int pin, uint8_t mode = 0xFF, bool needAnalog = false)
  {
    if (!valid(pin))
      return false;
    if (needAnalog && !isAnalogCapable(pin))
      return false;
    if (owner[pin] != -1)
      return false;
    owner[pin] = 1;
...
bool cmdWrite(const char *command)
{
  ...
  switch (ct)
  {
  case COMP_DIGITAL_OUT:
    return (c.type == COMP_DIGITAL_OUT) ? commandDigitalWrite(pin, command) : false;
  case COMP_PWM_OUT:
    return (c.type == COMP_PWM_OUT) ? commandPWMWrite(pin, command) : false;
  case COMP_RGB:
    return (c.type == COMP_RGB) ? commandRGB(c, command) : false;

#if ENABLE_SERVO
  case COMP_SERVO:
    return (c.type == COMP_SERVO) ? commandMoveServo(c, command) : false;
#endif

#if ENABLE_LCD
  case COMP_LCD:
    return (c.type == COMP_LCD) ? commandLCDScreen(c, command) : false;
#endif

#if ENABLE_STEPPER
  case COMP_STEPPER:
    return (c.type == COMP_STEPPER) ? commandStepper(command) : false;
#endif

#if ENABLE_LED_STRIP
  case COMP_LED_STRIP:
    return (c.type == COMP_LED_STRIP) ? commandLedStrip(command) : false;
#endif

#if ENABLE_LED_MATRIX
  case COMP_LED_MATRIX:
    return (c.type == COMP_LED_MATRIX) ? commandLedMatrix(command) : false;
#endif

#if ENABLE_TM
  case COMP_TM:
    return (c.type == COMP_TM) ? commandTmDisplay(command) : false;
#endif

#if ENABLE_BUZZER
  case COMP_BUZZER:
    return (c.type == COMP_BUZZER) ? commandBuzzer(pin, command) : false; // this one still needs pin
#endif

#if ENABLE_MOTOR
  case COMP_MOTOR:
    return (c.type == COMP_MOTOR) ? commandMotor(c, command) : false;
#endif
...
bool handleRegister(const char *command)
{
  ...
  case COMP_DIGITAL_IN:
    ok = makeDigitalIn(component, pin);
    break;
  case COMP_BUTTON:
    ok = makeButton(component, pin);
    break;
  case COMP_DIGITAL_OUT:
    ok = makeDigitalOut(component, pin);
    break;
  case COMP_ANALOG_IN:
    ok = makeAnalogIn(component, pin);
    break;
  case COMP_PWM_OUT:
    ok = makePWMOut(component, pin);
    break;
  case COMP_RGB:
    ok = makeRGB(component, command);
    break;
  case COMP_JOYSTICK:
    ok = makeJoystick(component, command);
    break;
  case COMP_ULTRASONIC:
    ok = makeUltrasonic(component, command);
    break;
  case COMP_THERMISTOR:
    ok = makeThermistor(component, command);
    break;

#if ENABLE_SERVO
  case COMP_SERVO:
    ok = makeServo(component, pin);
    break;
#endif

#if ENABLE_LCD
  case COMP_LCD:
    ok = makeLCD(component, command);
    break; // parses cols/rows/addr internally
#endif

#if ENABLE_DHT
  case COMP_DHT:
    ok = makeDHT(component, pin, command);
    break;
#endif

#if ENABLE_IR_REMOTE
  case COMP_IR_REMOTE:
    ok = makeIRRemote(component, pin);
    break;
#endif

#if ENABLE_RFID_UART
  case COMP_RFID_UART:
    ok = makeSoftUart(component, COMP_RFID_UART, command);
    break;
#endif

#if ENABLE_STEPPER
  case COMP_STEPPER:
    ok = makeStepper(component, command);
    break;
#endif

#if ENABLE_LED_STRIP
  case COMP_LED_STRIP:
    ok = makeLEDStrip(component, pin, command);
    break;
#endif

#if ENABLE_LED_MATRIX
  case COMP_LED_MATRIX:
    ok = makeLedMatrix(component, pin, command);
    break;
#endif

#if ENABLE_BUZZER
  case COMP_BUZZER:
    ok = makePassiveBuzzer(component, pin);
    break;
#endif

#if ENABLE_TM
  case COMP_TM:
    ok = makeTM(component, command);
    break;
#endif

#if ENABLE_MOTOR
  case COMP_MOTOR:
    ok = makeMotor(component, command);
    break;
#endif
...
bool makePassiveBuzzer(Component &out, int buzzerPin)
{
  ...
  {
    if (idx >= 0 && idx < MAX_SERVO)
      gServoUsed[idx] = false;
  }
...
bool makeLCD(Component &out, const char *command)
{
  ...
  if (!PinManager::reserve(a4, 0xFF, true))
    return false;
  if (!PinManager::reserve(a5, 0xFF, true))
    return false;
...
bool commandLCDScreen(Component &component, const char *command)
{
  ...
  bool makeDHT(Component &out, int pin, const char *command)
  {
    ...
    if (!PinManager::reserve(pin, INPUT))
      return false;
  ...
  bool makeSoftUart(Component &out, ComponentType t, const char *command)
  {
    ...
    {
      if (PinManager::valid(rx))
        PinManager::release(rx);
      if (PinManager::valid(tx))
        PinManager::release(tx);
      return false;
    }
    // if (t == COMP_BT_UART) {
  ...
  static RfidState dataParser(RfidState s, uint8_t c)
  {
    ...
    case READING_DATA:
    {
      ...
      if (++rfid_g_nibble < 12)
      {
        int n = hexNib(c);
        if (n < 0)
          return WAITING_FOR_STX; // invalid char -> resync
  ...
  bool makeStepper(Component &out, const char *command)
  {
    ...
    {
      if (PinManager::valid(pin1))
        PinManager::release(pin1);
      if (PinManager::valid(pin2))
        PinManager::release(pin2);
      if (PinManager::valid(pin3))
        PinManager::release(pin3);
      if (PinManager::valid(pin4))
        PinManager::release(pin4);
      return false;
    }
  ...
  bool makeMotor(Component &out, const char *command)
  {
    ...
    out = {};
    if (!PinManager::reserve(en1Pin, OUTPUT))
      return false;
    if (!PinManager::reserve(in1Pin, OUTPUT))
      return false;
    if (!PinManager::reserve(in2Pin, OUTPUT))
      return false;
    ...
    if (numberMotors == 2)
    {
      ...
      if (!PinManager::reserve(en2Pin, OUTPUT))
        return false;
      if (!PinManager::reserve(in3Pin, OUTPUT))
        return false;
      if (!PinManager::reserve(in4Pin, OUTPUT))
        return false;
      out.pins[3] = en2Pin;
      out.pins[4] = in3Pin;
      out.pins[5] = in4Pin;
  ...
  bool cmdNeoFrame(const char *command)
  {
    ...
    if (!data.valid())
      return false;
    if (data.len != (uint16_t)n * 6)
      return false; // exact length required

    for (uint8_t i = 0; i < n; i++)
    {
      const char *s = data.ptr + (uint16_t)i * 6;
      uint8_t r, g, b;
      if (!hex6ToRGB(s, r, g, b))
        return false;
  ...
  bool setIndividualColor(const char *command)
  {
    ...
    if (!colorToken.valid() || colorToken.len < 6)
      return false;
    if (position >= neoPixel->numPixels())
      return false;

    uint8_t r, g, b;
    const char *colorPtr = colorToken.ptr;

    if (!hex6ToRGB(colorPtr, r, g, b))
      return false;
  ...
  bool makeLedMatrix(Component &out, int dinPin, const char *command)
  {
    ...
    if (!PinManager::reserve(dinPin, OUTPUT))
      return false;
    if (!PinManager::reserve(csPin, OUTPUT))
      return false;
    if (!PinManager::reserve(clkPin, OUTPUT))
      return false;
  ...
  bool makeTM(Component &out, const char *command)
  {
...
bool makeRGB(Component &out, const char *command)
{
  ...
  if (!PinManager::reserve(redPin, OUTPUT))
    return false;
  if (!PinManager::reserve(greenPin, OUTPUT))
    return false;
  if (!PinManager::reserve(bluePin, OUTPUT))
    return false;
  out = {};
  out.type = COMP_RGB;
  out.nPins = 3;
...
bool makeThermistor(Component &out, const char *command)
{

  nextToken(&command);

  int apin = nextToken(&command).asPin();
  if (!PinManager::reserve(apin, 0xFF, true))
    return false;
  out = {};
...
{
  if (!PinManager::reserve(pin, INPUT_PULLUP))
    return false;
  out = {};
...
{
  if (!PinManager::reserve(apin, 0xFF, /*needAnalog*/ true))
    return false;
  out = {};
...
bool makeJoystick(Component &out, const char *command)
{
  ...
  if (!PinManager::reserve(axPin, 0xFF, true))
    return false;
...
bool makeUltrasonic(Component &out, const char *command)
{
  ...
  if (!PinManager::reserve(trig, OUTPUT))
    return false;
